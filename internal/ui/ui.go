...
func keyLegendDescs(emoji bool) map[string]string {
	...
	return map[string]string{
		...
		"c":      "Copy the selected port's URL to the clipboard, via OSC 52 so it\nworks even over SSH (needs a terminal that supports it; tmux: set -g\nset-clipboard on). The URL matches how the port is reachable: a\nPUBLISHED port copies its public https://<hostname>, a LAN-bound port\nits http://<lan-ip>:<port>, a localhost-only or offline port\nhttp://localhost:<port>, and everything else (served/tailnet, and a\nfunnelled port) the tailnet http://<host>:<port>. Copies even before\nit's served — the toast says so.",
