...
echo "== (d) 0.x-minor upgrade is NON-breaking now: 0.1.0 -> 0.2.0 (mmgv) =="
HD="$WORKDIR/homeD"
mkdir -p "$HD"
...
assert_rc_zero "0.x-minor upgrade exit 0 (non-breaking under mmgv)"
assert_installed "$HD/.local/bin/tailport" tailport-v0.2.0
assert_bak_matches "$HD/.local/bin/tailport.bak" "$old_d" ".bak holds old 0.1.0 binary"
assert_out_contains "v0.1.0 -> v0.2.0" "prints old -> new (0.1.0 -> 0.2.0), no opt-in needed"
...
echo "== (g) breaking downgrade across a MAJOR boundary is gated: 2.0.0 -> 1.2.0 (mmgv) =="
HG1="$WORKDIR/homeG1"
mkdir -p "$HG1"
run_install sh "$REL200" "HOME=$HG1" >/dev/null 2>&1 || fail "seed 2.0.0 failed (g1)"
old_g1="$(sha256_of "$HG1/.local/bin/tailport")"
capture_install sh "$REL120" "HOME=$HG1"
assert_rc_nonzero "major breaking downgrade refused (exit nonzero)"
