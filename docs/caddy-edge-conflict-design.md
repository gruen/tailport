...
- (C) a final **full-array scan** (NOT `InspectConflict`, which short-circuits on our `@id` — roborev-k7br-#2) that confirms three things for the message (roborev-e4ne-#1): (i) a route for the hostname is **present**, (ii) it **semantically equals `captured.Raw`** — a normalized-JSON compare including fields tailport doesn't model, so a same-`@id` `PATCH` between B and C that swapped the backend/auth is NOT reported as "restored" — and (iii) **no other** route overlaps the hostname. All three ⇒ "restored"; otherwise the corresponding honest message (belt-and-suspenders with B's own scan-before-append).
...
- **Order-fidelity: content restored, position not (accepted loss — corrected per roborev-k7br-#2).** Owned-only undo removes rev-1's WORST case (an id-less foreign route restored by a now-meaningless array index — that never occurs, an owned route always has an `@id`). But an `@id` preserves a route's *content*, not its *array position*: `RestoreRoute` POST-**appends**, so the restored route lands at the array END, and Caddy evaluates routes in order — so its precedence relative to a catch-all or other non-overlapping route can differ from before the purge. This is often invisible (distinct host matchers don't shadow each other), but it is **not** unconditionally benign (roborev-he97-#1): Caddy evaluates routes **in array order**, so a **preceding `Terminal:true` catch-all** (a `match: *` or a broader host that the edge lists before our route) would **intercept the hostname before** our re-appended route ever runs — the undo could report "restored" while the name now routes to the catch-all's backend. So the loss is real on an edge that leans on route ordering. v1 **accepts** it (re-append) rather than attempt a stale-index `PUT .../routes/<origIndex>` insert, and the undo's success message **claims content restored, not positional/routing fidelity**. Test it explicitly with a preceding terminal catch-all so the limitation is pinned, not assumed away.
...
5. ~~**OQ5 — id-less-foreign order fidelity**~~ — **DISSOLVED by OQ8**: the id-less-foreign case (restore by a now-meaningless array index) never occurs — an owned route always has an `@id`. Note this eliminates the *id-less* case but **not** the append-order loss for owned routes: `RestoreRoute` still POST-appends, so a restored owned route's position (and thus precedence vs a preceding terminal catch-all) can change — an accepted, documented loss (§3.6), not "no precedence loss."
