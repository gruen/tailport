...
| §5 Smoke test | The definitive end-to-end check — `curl https://<hostname>/` from a machine **off** your tailnet — is provider-agnostic and unchanged; that's the real proof. §5's *in-edge* probes only localize a failure, and must run in the edge's own network context: `tailscale ping` via `docker exec` (the CLI is in the image); and, since the minimal image ships no `curl` (bash + CA certs only), the `curl -H "Host: …"` probe from a curl-capable **sidecar joined to the edge's network namespace** (`docker run --network container:<edge> …`) — not the host, whose routing/resolver differ — or a plain host shell for a native systemd deploy, where the host *is* the edge. |
