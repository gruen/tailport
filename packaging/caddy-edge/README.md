...
| [`bootstrap-caddy.json`](./bootstrap-caddy.json) | Caddy's first-boot admin config: an empty `tailport` HTTP server on `:443`/`:80`, plus a widened `admin.origins` list (see below). |
...
`bootstrap-caddy.json` widens `admin.origins` to include that address (and
the full `<hostname>.<tailnet>.ts.net:<admin_port>` form, in case a caller
addresses the edge by its FQDN instead of the short MagicDNS label). JSON
has no comment syntax, so this note — and the fact that
`caddy.<tailnet>.ts.net:2019` is a **placeholder you must edit** before
building the image — lives here instead of inline. Replace `<tailnet>` with
your actual tailnet name (visible in the Tailscale admin console, or via
`tailscale status` on any node — it's the part between the hostname and
`.ts.net`). If you changed `caddy.hostname` away from the default `caddy`,
also replace the literal `caddy:2019` / `caddy.<tailnet>...` entries with
...
**Verified here:** `bootstrap-caddy.json` is valid JSON
...
directory was run for real: both build stages complete, the multistage copy
of `tailscale`/`tailscaled` off the official Tailscale image onto
`caddy:2-alpine` succeeds, `bootstrap-caddy.json`/`entrypoint.sh` copy in and
...
*accepts* `bootstrap-caddy.json` (as opposed to it merely being well-formed
